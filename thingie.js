...
function setup() {
    ...
    attackSlide.mouseReleased(updateSettings);
    ...
    decaySlide.mouseReleased(updateSettings);
    ...
    sustainSlide.mouseReleased(updateSettings);
    ...
    releaseSlide.mouseReleased(updateSettings);
    ...
    volumeSlide.mouseReleased(updateSettings);
...
function draw() {
    ...
    // //
    // for (var note in synths) {
    //     synths[note] = flock.synth({
    //         synthDef: {
    //             ugen: "flock.ugen.square",
    //             freq: dict[note],
    //             mul: {
...
function updateSettings() {
    ...
    // note: this isn't working right now. need to look into how to change ADSR on the fly
    // for (var note in synths) {
    //     synths[note].set("options.synthDef", {
    //         ugen: "flock.ugen.square",
    //         freq: dict[note],
    //         mul: {
    //             id: "env",
...
function keyPress(note) {
    synths[note].set("env.gate", 1); // turn env on
}
...
function keyRelease(e) {
    ...
    if (key in keys) {
        ...
        var note = keys[key];
        synths[note].set("env.gate", 0); // turn env off
    }
}
...
document.onmouseup = function() {
    for (var key in synths) {
        synths[key].set("env.gate", 0); // turn env off
